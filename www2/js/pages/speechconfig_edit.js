...
function msgCallback(data) {
    ...
    if (obj.hasOwnProperty("method") && obj.hasOwnProperty("payload")) {
        if (obj.method == "getSortsCb") {
        ...
        } else if (obj.method == "editSpeechConfigCb") {
            if (obj.payload.result == 0) {
                showMessager("修改成功", obj.payload.result);
                closeLoading();
                back();
            }
        } else if (obj.method == "getSpeechConfigCb") {
            $("#form-speechconfig-edit").setform(obj.payload);
            ...
            if (obj.payload.index == obj.payload.total) {
                ...
                getSceneConfigs();
                getCombineScenes();
                getSpeechConfig(getUrlParam("word_id"));
            }
        }
    }
}

function editSpeechConfig() {
    //表单验证
    ...
    openLoading(10000);
    doEditSpeechConfig(word_id, words, room_id, getActionList());
}


function getSpeechConfig(word_id) {
    doGetSpeechConfig(word_id);
}
